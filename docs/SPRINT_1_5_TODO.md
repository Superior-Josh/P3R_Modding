...
>
> 目的：记录 Zen byte-patch 工程化阶段的状态与遗留项。
...
Sprint 1.5 的核心目标是替代传统 `.uasset+.uexp` 写回，建立可工作的 Zen byte-patch 链。当前主路径已交付，并成为项目默认方案。

## 遗留项

- 持续增加模板解析覆盖。
- 对 PARTIAL schema 添加字段级人工复核状态。
- 将更多 DSL helper 绑定到 guard 放行字段。
- 补充更多游戏内验证样例。
