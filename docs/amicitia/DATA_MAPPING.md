...
| Wiki/需求 | TableKey / 资产 |
|---|---|
| Skills 数值 | `Skills` → `DatSkillNormalDataAsset.uasset` |
| Skills 元数据 | `SkillData` → `DatSkillDataAsset.uasset` |
| Personas | `Persona` / `PersonaGrowth` / `PersonaAffinity` |
| Enemies | `Enemy` / `EnemyAffinity` |
| Encounters | `EncountTable` |
| Items/Weapons/Armor/Accessories | `Xrd777/UI/Tables/DatItem*DataAsset.uasset` |
| Difficulty | `DT_BtlDIfficultyParam` named rows |

## 资产定位原则

- 同名资产默认优先 Xrd777。
- Wiki ID 不能直接替代字段 offset；写回必须经过 JSON/schema/guard 验证。
- 没有 TableKey 的页面仅作为研究参考，不代表当前自动写回支持。
