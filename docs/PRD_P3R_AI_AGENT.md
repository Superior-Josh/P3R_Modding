...
构建一个**自然语言驱动的 P3R Mod 制作 AI Agent**，让硬核玩家/模组爱好者只需说出想要的效果（如"把亚基的伤害改成 999"），Agent 自动完成：定位文件 → 读取数据 → 展示差分 → 修改写回 → 打包 PAK — **全程无需手动操作**。
...
│  ├─ a. 克隆原始 JSON → 修改 Data[10].hpn = 999                    │
...
| V3 | 数值修改写回 | 修改 Skills Data[10].hpn=999 → .Write() → 再次 UAssetAPI 加载 → 确认值=999 | 自动化断言 |
...
| # | 测试项 | 预期结果 | 验证方法 |
|---|--------|---------|---------|
| V12 | 自然语言全流程 | "把亚基伤害改成999" → Agent 自动完成 → .pak 生成 | 端到端手动测试 |
