...
>
> 目的：概述当前项目识别的 P3R 资产结构、DataTable 与 Mod 入口。
...
P3R 使用 UE 4.27，主要资产来自 IoStore (`.utoc/.ucas`)。Mod 不修改原始容器，而是在 UE 虚拟路径上覆盖同名资产。
...
| 道具/装备 | `Xrd777/UI/Tables/DatItem*DataAsset.uasset` |
| 难度参数 | `DT_BtlDIfficultyParam` |

## Xrd777 优先级

存在 Xrd777 与 Astrea 同名/相近资产时，以 Xrd777 为默认主路径；除非有明确证据表明目标来自 Astrea。
