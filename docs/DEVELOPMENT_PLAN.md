...
## Sprint 2: 工具链集成（Zen patch 主路径）

> **目标**: 编写 Claude Code 工具脚本与 schema guard，把自然语言请求安全落到 Sprint 1.5 的 Zen byte-patch 管道：解析需求 → 定位 ID/字段 → DryRun 预览 offset → guard → `modify-and-repack.ps1` → UnrealEssentials 散文件 Mod。
> **工期**: 26h | **依赖**: Sprint 1.5 完成 | **可交付物**: 全部工具可用 + schema/field guard + 完整 CLAUDE.md

### 任务清单

| ID | 任务 | 描述 | 工时 | 依赖 | Claude Code 做什么 | 人工做什么 |
|----|------|------|------|------|-------------------|-----------|
| **T2.1** | Config.ps1 扩展 | 添加 `$SchemaDir`、`$ZenSourceRoot`、`$ModRegistry`、`$BackupDir`、`$ToolsDir` 变量；统一 TableKey → VirtualPath → SchemaKey 解析 | 2h | T1.5 | 生成 Config.ps1 新增部分 | 审查并加载测试 | ✅ |
| **T2.2** | search-datatable.ps1 | 实现：查 DATA_MAPPING.md + docs/zh-cn + Wiki MD + JSON 缓存 → 返回虚拟路径、TableKey、SchemaKey、行索引、字段、当前值 | 4h | T2.1 | 编写搜索脚本（~200 行 PS），含中文模糊匹配逻辑 | 测试各种查询（中/日/英名称、拼音、描述） | ✅ |
| **T2.3** | search-wiki.ps1 | 实现：grep 37 个 Wiki MD 与 docs/zh-cn → 返回相关条目 + 对应游戏文件路径/ID | 2h | — | 编写 Wiki 搜索脚本（~100 行 PS），含索引缓存优化 | 测试查询质量 | ✅ |
| **T2.4** | diff-changes.ps1 | 实现：把 changes 计划转成人类可读输出（含 Wiki/中文名称、旧值→新值、schemaKey、target） | 4h | T2.2 | 编写 diff 脚本（~180 行 PS），含 ID→名称翻译 | 测试各种类型修改的预览效果 | ✅ |
| **T2.5** | backup-mod.ps1 | 实现：创建时间点备份 → 复制原始 Zen `.uasset`、changes.json、ModConfig.json → 写入 backup.json | 2h | T2.1 | 编写备份脚本（~100 行 PS） | 测试备份/恢复 | ✅ |
| **T2.6** | rollback-mod.ps1 | 实现：查找 mod 注册表 → 展示信息 → 确认 → 移除 Reloaded II Mod 目录或 UnrealEssentials 产物 → 验证 | 3h | T2.5 | 编写回滚脚本（~150 行 PS） | 端到端测试回滚 | ✅ |
| **T2.7** | conflict-check.ps1 | 实现：遍历已安装/已生成 mod 的 changes.json/mod.json → 对比同一 virtualPath + target/row/field → 报告重叠 | 3h | T2.1 | 编写冲突检测脚本（~150 行 PS） | 创建冲突场景测试 | ✅ |
| **T2.8** | schema-guard.ps1 / guard 集成 | 根据 schema 回归状态和字段元数据拦截：FAIL/SKIP、PARTIAL 未复核字段、union、nested struct array、string/TArray/变长字段 | 4h | T1.5.4, T2.1 | 编写 guard 逻辑并集成到 `modify-and-repack.ps1 -DryRun` 输出 | 人工核查 9 个 PARTIAL 的提示是否准确 | ✅ |
| **T2.9** | modify-and-repack.ps1 集成增强 | 在 Sprint 1.5 已完成的 Zen 管道上接入 search/diff/backup/conflict/guard/registry；改善错误输出，不再集成 `create` | 2h | T2.5-T2.8 | 扩展编排脚本 | 全流程测试 | ✅ |
| **T2.10** | CLAUDE.md 更新 | 添加全部工具定义、Zen patch 工作流规则、安全协议、unsupported/requiresManualResearch 规则、使用示例 | 2h | T2.2-T2.9 | 生成 CLAUDE.md 新增章节 | 审查并确认 Claude Code 正确加载 | ✅ |
| **T2.11** | Sprint 2 评审 | 端到端 AI 测试：自然语言输入 → Agent 自动完成 → UnrealEssentials Zen 散文件 Mod 生成；对不安全目标正确拒绝 | 1h | T2.10 | — | 用 5+ 种不同需求的自然语言做完整测试 | ✅ |

### 交付物

- [x] `tools/scripts/Config.ps1` 更新（Schema/Zen/Registry 变量）
- [x] `tools/scripts/tools/search-datatable.ps1`
- [x] `tools/scripts/tools/search-wiki.ps1`
- [x] `tools/scripts/tools/diff-changes.ps1`
- [x] `tools/scripts/tools/backup-mod.ps1`
- [x] `tools/scripts/tools/rollback-mod.ps1`
- [x] `tools/scripts/tools/conflict-check.ps1`
- [x] `tools/scripts/tools/guard-modify.ps1`（含 schema/field guard）
- [x] `tools/scripts/modify-and-repack.ps1` 集成增强（基于 Sprint 1.5 Zen 管道）
- [x] `CLAUDE.md` 更新（含工具定义 + 安全规则）
- [x] 端到端测试报告（5+ 自然语言场景 + 3+ unsupported 场景）

### 任务依赖图

```
T2.1 (Config/Schema解析)
  │
  ...
  ├──→ T2.7 (conflict)               │
  │                                  │
  └──→ T2.8 (schema/field guard) ────┤
                                     ↓
T2.3 (search-wiki) ───────────→ T2.9 (modify-and-repack集成增强)
                                     │
                                     └──→ T2.10 (CLAUDE.md) ──→ T2.11 (评审)
```
...
| **T3.7** | 安全屏障脚本增强 | 增强 `guard-modify.ps1`：备份存在、无冲突、值合法、schema 状态、field-level 安全性、Zen 输出大小不变、无 `.uexp` | 2h | T3.2-T3.6, T2.8 | 编写安全屏障脚本增强（~100 行 PS） | 集成到 modify-and-repack 流程 | ✅ |
...
- [x] `tools/Output/mod/<name>/mod.json` 注册表格式
- [x] `tools/Output/mod/<name>/history.json` 审计日志
- [x] `tools/scripts/tools/backup-mod.ps1` 增强版
- [x] `tools/scripts/tools/rollback-mod.ps1` 增强版
- [x] `tools/scripts/tools/conflict-check.ps1` 增强版
- [x] `tools/scripts/tools/guard-modify.ps1` 安全屏障
- [x] `docs/SECURITY.md` 安全协议文档
- [x] 安全系统测试报告（`docs/SPRINT_3_TEST_REPORT.md`；非破坏性 smoke 通过，真实游戏验证保留人工项）
...
> **目标**: 扩展 Zen patch 覆盖率、完善多表/批量能力、端到端验证、用户文档编写
...
| ID | 任务 | 描述 | 工时 | 依赖 | Claude Code 做什么 | 人工做什么 |
|----|------|------|------|------|-------------------|-----------|
| **T4.1** | 多表 Mod 支持 | 修改 `modify-and-repack.ps1` 支持一次修改多个 DataTable，逐表生成 Zen `.uasset` 并镜像到 UnrealEssentials 路径 | 3h | T2.9 | 扩展编排脚本（+100 行 PS） | 测试 2-3 表联合修改 |
| **T4.2** | 010 schema 覆盖验证 | 复跑全部 schema regression，整理 PASS/PARTIAL/FAIL/SKIP；为可自动写回字段生成 allowlist，为风险字段生成 guard metadata | 3h | T2.8 | 生成批量验证脚本/报告 | 审查 9 个 PARTIAL 与 2 个 FAIL |
| **T4.3** | 批量修改支持 | 实现 "将所有 X 的 Y 改为 Z" 模式（筛选 + 批量 changes + 单次 Zen patch 部署） | 3h | T4.1, T4.2 | 编写批量修改脚本（~150 行 PS） | 测试批量操作正确性 |
| **T4.4** | 边界情况测试 | 测试：空行、最大值溢出、负值、无效 ID、路径空格、schema mismatch、file size mismatch、unsupported target | 3h | T4.3 | 生成边界测试用例脚本 | 逐项执行并记录 |
| **T4.5** | 用户文档 | 编写 `docs/USER_GUIDE.md`：安装、首次使用、常见场景、FAQ、unsupported/requiresManualResearch 说明 | 2h | T4.4 | 生成用户文档初稿 | 审查、补充截图和示例 |
| **T4.6** | Sprint 4 评审 | 全量端到端测试 + 文档完整性检查；至少覆盖 Agi/Bufu/Exp、多表、批量、拒绝危险 target | 2h | T4.5 | 生成最终测试报告 | 逐项验收 |

### 交付物

- [ ] `tools/scripts/modify-and-repack.ps1` 多表支持版
- [ ] `tools/scripts/tools/batch-modify.ps1` 批量修改脚本
- [ ] 010 schema 验证报告（PASS/PARTIAL/FAIL/SKIP + field-level allowlist/denylist）
- [ ] 边界测试报告（含 schema mismatch / file size mismatch / unsupported target）
...
Sprint 0 ──→ Sprint 1 ⊘ ──→ Sprint 1.5 ★ ──→ Sprint 2 ──→ Sprint 3 ──→ Sprint 4
  │              │                │              │            │            │
  │              │                │              │            │            └── 多表/批量/010 schema覆盖/文档
  │              │                │              │            └── 备份/回滚/冲突/审计 + guard增强
  │              │                │              └── 自然语言工具 + schema/field guard + Zen管道集成
  │              │                └── ★ Zen byte-patch 写回（Agi/Bufu/ExpMod 已验证）
  │              └── ⊘ UAssetAPI写回 + 游戏验证（弃用，P3R 崩游戏，P-007）
  └── 模板库 + setup + 开发环境（templates/ 部分弃用，setup/ 仍有效）
```

**关键路径**: Sprint 0 T0.4-T0.6（保留有效部分）→ Sprint 1.5 T1.5.5/T1.5.7（Zen patch CLI + 管道）→ Sprint 2 T2.8/T2.9（schema guard + 自然语言工具集成）
...
| 风险 | 概率 | 额外工时 | 触发条件 |
|------|------|---------|---------|
| PARTIAL/FAIL/SKIP schema 被自然语言 Agent 误判为可安全写回 | 25% | +8h | guard 未读取 regression metadata 或 field-level 状态缺失 |
| union / nested struct array target 被直接 byte-patch 导致游戏崩溃 | 20% | +10h | 用户请求 PersonaGrowth 技能槽等含 union 结构 |
| string / TArray / 变长字段需求无法用 in-place patch 表达 | 20% | +12h | 用户要求改文本、增删行、改数组长度 |
| P3R 游戏更新后 Zen 文件大小/headerSize 改变 | 15% | +8h | schema regression 黄金锚点或 file size 校验失败 |
| UnrealEssentials 路径/依赖配置错误导致 mod 不生效 | 15% | +4h | ModConfig.json 依赖、SupportedAppId 或虚拟路径镜像错误 |

**建议**: Sprint 2/3 为 schema guard 与真实游戏 smoke test 预留 20% 缓冲。传统 `.uasset+.uexp` / PAK 路线已被 P-007 证伪，不再作为风险缓冲主线。
...
```
Week 1 ─ Sprint 0: 基础设施（历史阶段）
  Day 1-2  ██ 传统模板导出 + 验证（现已降级为 fallback）
  Day 3-4  ██ setup.ps1 + 文档
  Day 5    ██ Sprint 0 评审
  ───────────────────────────
      里程碑 A: 读取/环境基础就绪；传统模板仅保留备查

Week 2-3 ─ Sprint 1: 传统写回引擎（历史阶段，已弃用）
  Day 1-3  ██ TemplateCreator / create 命令
  Day 4-5  ██ 往返测试
  Day 6     ██ 游戏加载测试 → P-007 证伪
  Day 7-9  ██ 迁移决策
  Day 10   ██ Sprint 1 评审
  ───────────────────────────
      里程碑 B: 传统 `.uasset+.uexp` / PAK 路线弃用，转入 Sprint 1.5

Week 3-4 ─ Sprint 1.5: Zen byte-patch 写回（已完成）
  Day 1-3  ██ 010 schema 导入 / 解析 / header 校准
  Day 4-5  ██ schema regression / Invoke-ZenPatch
  Day 6-7  ██ DSL / modify-and-repack Zen 管道
  Day 8    ██ Agi/Bufu/ExpMod 人工验证
  ───────────────────────────
      里程碑 B2: Zen patch → UnrealEssentials 散文件主路径可用

Week 4-5 ─ Sprint 2: 工具链集成
  Day 1-3  ██ search + diff + wiki 脚本
  Day 4-6  ██ backup + rollback + conflict 脚本
  Day 7-8  ██ schema/field guard + modify-and-repack 集成增强 + CLAUDE.md
  Day 9-10 ██ 端到端 AI 测试 + unsupported 场景拒绝 + Sprint 2 评审
  ───────────────────────────
      里程碑 C: 自然语言 → Zen patch → UnrealEssentials 散文件全流程可用
...
| `tools/scripts/tools/guard-modify.ps1` | S2/S3 | 新建（S2 实现 schema/field guard，S3 增强审计/冲突/回滚联动） |
