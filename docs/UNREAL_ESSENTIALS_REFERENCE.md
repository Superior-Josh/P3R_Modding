...
>
> 目的：记录本项目使用 UnrealEssentials 的方式与限制。
...
```text
<Mod>/UnrealEssentials/P3R/Content/<虚拟路径剩余部分>/<Asset>.uasset
```

例如：

```text
AgiMod/UnrealEssentials/P3R/Content/Xrd777/Battle/Tables/DatSkillNormalDataAsset.uasset
```

## 关键限制

- 文件名与虚拟路径必须完全匹配。
- P3R IoStore 资产替换应使用来自 UTOC/IoStore 的 Zen 单文件。
- 传统 `.uasset+.uexp` 不作为当前推荐路径。
