...
本目录扫描到 48 个 `.bt` 文件。它们来自 010-Editor 模板生态，用于描述 P3R/UE 资产的二进制结构。

## 工作流

```powershell
.\tools\scripts\Parse-BtTemplate.ps1 -TemplatePath <template.bt> -OutputPath <schema.json>
...
模板能被解析不等于字段自动安全可改；最终以 schema regression 与 guard policy 为准。
