...
if ($NoPack) {
    ...
    # Pack with UnrealPak. The manifest already has absolute source paths
    # (generated by P3RDataTools), so it works from any directory.
    $unrealPakDir = Split-Path $UnrealPak -Parent
    Push-Location $unrealPakDir
    $result = & $UnrealPak $pakFile "-Create=$manifestFile" -compress 2>&1
    Pop-Location
